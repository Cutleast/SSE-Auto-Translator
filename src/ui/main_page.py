...
from PySide6.QtGui import QCursor, QFont, QIcon, QKeySequence
...
from core.database.database import Translation
from core.mod_managers import SUPPORTED_MOD_MANAGERS
from core.processor import Processor
from core.utilities import apply_dark_title_bar, relative_data_path
from core.utilities.constants import AE_CC_PLUGINS, BASE_GAME_PLUGINS
from core.utilities.importer import get_non_plugin_files, parse_path
...
from .widgets.ignore_list_dialog import IgnoreListDialog
from .widgets.loading_dialog import LoadingDialog
from .widgets.search_bar import SearchBar
from .widgets.shortcut_button import ShortcutButton
from .widgets.stacked_bar import StackedBar
from .widgets.string_list_dialog import StringListDialog
...
class MainPageWidget(QWidget):
    ...
    def __init__(self, app: MainApp):
        ...
        self.filter_none = QCheckBox(self.mloc.filter_none, filter_menu)
        ...
        self.filter_no_strings = QCheckBox(self.mloc.filter_no_strings, filter_menu)
        ...
        self.filter_translation_installed = QCheckBox(
        ...
        self.filter_translation_available = QCheckBox(
        ...
        self.filter_requires_translation = QCheckBox(
        ...
        self.filter_no_translation_available = QCheckBox(
        ...
        scan_modlist_action = self.tool_bar.addAction(
            QIcon(str(self.app.res_path / "icons" / "detect_lang.svg")),
        ...
        scan_online_action = self.tool_bar.addAction(
            QIcon(str(self.app.res_path / "icons" / "scan_online.svg")),
        ...
        self.bar_chart = StackedBar(
            [0 for s in Plugin.Status.get_members()],
            colors=[Plugin.Status.get_color(s) for s in Plugin.Status.get_members()],
        )
        ...
        self.mods_widget.header().setDefaultAlignment(Qt.AlignmentFlag.AlignCenter)
        ...
        def on_context_menu(point: QPoint):
            ...
            mouse_pos = self.mods_widget.mapFromGlobal(QCursor.pos())
            ...
            if current_item.isDisabled():
                return
            if (
                Qt.ItemFlag.ItemIsUserCheckable in current_item.flags()
                ...
                if matching:
                    ...
                    if mod.mod_id:
                        url = self.app.provider.get_modpage_link(
                            mod.mod_id, source=Source.NexusMods
                        )
                        os.startfile(url)
            ...
            def create_translation():
                if plugin_selected:
                    if not self.app.database.get_translation_by_plugin_name(
                        ...
                        selected_plugin.status = Plugin.Status.TranslationIncomplete
                        self.update_modlist()
                    ...
                    def process(ldialog: LoadingDialog):
                        ...
                        plugins = [
                            ...
                            in [
                                ...
                                Plugin.Status.NoTranslationAvailable,
                            ]
                        ]
                        ...
                        if existing_translation is None:
                            ...
                            non_plugin_files = get_non_plugin_files(
                                selected_mod.path,
                            ...
                            for file in non_plugin_files:
                                ...
                                if bsa:
                                    ...
                                    shutil.copyfile(
                                        selected_mod.path / relative_data_path(file),
                                        translation / "data" / relative_data_path(file),
                                    )
                            ...
                            for bsa, files in bsa_files_to_extract.items():
                                ...
                                for file in files:
                                    parsed_bsa.extract_file(
                                        file, translation.path / "data"
                                    )
            ...
            if mod_selected:
                ...
                if any(
                    ...
                    or plugin.status == Plugin.Status.RequiresTranslation
                    for plugin in selected_mod.plugins
                ):
            ...
            if show_translation_actions:
                ...
                if plugin_selected:
                    if selected_plugin.status == Plugin.Status.TranslationIncomplete:
                ...
                create_translation_action.setDisabled(
                    selected_plugin.status == Plugin.Status.NoneStatus
                    or selected_plugin.status == Plugin.Status.NoStrings
                    or selected_plugin.status == Plugin.Status.TranslationInstalled
                )
        ...
        self.mods_widget.setContextMenuPolicy(Qt.ContextMenuPolicy.CustomContextMenu)
    ...
    def update_modlist(self, *args):
        ...
        def process_mod_item(mod_item: QTreeWidgetItem) -> bool:
            global none_status_plugins
            global no_strings_plugins
            global translation_installed_plugins
            global translation_available_plugins
            ...
            for plugin in mod.plugins:
                ...
                if plugin.name.lower() in ignore_list:
                    ...
                    plugin.tree_item.setCheckState(0, Qt.CheckState.Unchecked)
...
<tr><td><font color="{Plugin.Status.get_color(
    Plugin.Status.TranslationInstalled
).name()}">{self.mloc.translation_installed}:\
</font></td><td align=right><font color="{Plugin.Status.get_color(
    Plugin.Status.TranslationInstalled
).name()}">{translation_installed_plugins}</font></td></tr>

<tr><td><font color="{Plugin.Status.get_color(
    Plugin.Status.TranslationAvailableOnline
...
"""
    ...
    def load_mods(self):
        ...
        cur_separator: QTreeWidgetItem = None

        for i, mod in enumerate(modlist):
            if mod.name.endswith("_separator"):
                cur_separator = QTreeWidgetItem(
                    [
                ...
                cur_separator.setTextAlignment(0, Qt.AlignmentFlag.AlignCenter)
                cur_separator.setTextAlignment(2, Qt.AlignmentFlag.AlignCenter)
                font = QFont()
                font.setBold(True)
                font.setItalic(True)
                cur_separator.setFont(0, font)
                cur_separator.setFlags(Qt.ItemFlag.ItemIsSelectable)
    ...
    def show_help(self):
        ...
        none_status_label = QLabel(self.mloc.none_status)
        flayout.addRow(none_status_label)

        no_strings_label = QLabel(self.mloc.no_strings)
        flayout.addRow(no_strings_label)

        translation_installed_color = QLabel()
        translation_installed_color.setPixmap(
            qta.icon(
                "mdi6.square-rounded",
                color=Plugin.Status.get_color(Plugin.Status.TranslationInstalled),
            ).pixmap(32, 32)
        )
        flayout.addRow(self.mloc.translation_installed, translation_installed_color)

        translation_available_color = QLabel()
        translation_available_color.setPixmap(
            qta.icon(
                "mdi6.square-rounded",
                color=Plugin.Status.get_color(Plugin.Status.TranslationAvailableOnline),
            ).pixmap(32, 32)
        )
        flayout.addRow(self.mloc.translation_available, translation_available_color)

        translation_incomplete_color = QLabel()
        translation_incomplete_color.setPixmap(
            qta.icon(
                "mdi6.square-rounded",
                color=Plugin.Status.get_color(Plugin.Status.TranslationIncomplete),
            ).pixmap(32, 32)
        )
        flayout.addRow(self.mloc.translation_incomplete, translation_incomplete_color)

        requires_translation_color = QLabel()
        requires_translation_color.setPixmap(
            qta.icon(
                "mdi6.square-rounded",
                color=Plugin.Status.get_color(Plugin.Status.RequiresTranslation),
            ).pixmap(32, 32)
        )
        flayout.addRow(self.mloc.requires_translation, requires_translation_color)

        no_translation_available_color = QLabel()
        no_translation_available_color.setPixmap(
            qta.icon(
                "mdi6.square-rounded",
                color=Plugin.Status.get_color(Plugin.Status.NoTranslationAvailable),
            ).pixmap(32, 32)
        )
        flayout.addRow(
    ...
    def run_string_search(self):
        ...
        type_box.stateChanged.connect(
            lambda state: type_entry.setEnabled(state == Qt.CheckState.Checked.value)
        )
        ...
        formid_box.stateChanged.connect(
            lambda state: formid_entry.setEnabled(state == Qt.CheckState.Checked.value)
        )
        ...
        edid_box.stateChanged.connect(
            lambda state: edid_entry.setEnabled(state == Qt.CheckState.Checked.value)
        )
        edid_box.clicked.connect(edid_entry.setFocus)
        flayout.addRow(edid_box, edid_entry)

        string_box = QCheckBox(self.loc.main.string)
        string_entry = QLineEdit()
        string_entry.setDisabled(True)
        string_box.stateChanged.connect(
            lambda state: string_entry.setEnabled(state == Qt.CheckState.Checked.value)
        )
